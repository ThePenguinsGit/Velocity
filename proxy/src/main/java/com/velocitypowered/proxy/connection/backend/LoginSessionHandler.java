...
public class LoginSessionHandler implements MinecraftSessionHandler {
  ...
  public boolean handle(LoginPluginMessagePacket packet) {
    ...
    if (configuration.determineForwardingMode(serverName) == PlayerInfoForwarding.MODERN
  ...
  public boolean handle(ServerLoginSuccessPacket packet) {
    String serverName = serverConn.getServerInfo().getName();
    if (server.getConfiguration().determineForwardingMode(serverName) == PlayerInfoForwarding.MODERN && !informationForwarded) {
  ...
  public void disconnected() {
    String serverName = serverConn.getServerInfo().getName();
    if (server.getConfiguration().determineForwardingMode(serverName) == PlayerInfoForwarding.LEGACY) {
